...
| 周报 | 每周日 19:45 生成 / 20:15 阅读（自动） | `digests/YYYY-Www.md`（必须看 + 值得看精选，15 分钟可扫完） |
...
> **分支自动切换**：整条链路都必须在 `day-info` 分支上跑，但不需要你手动切。
>
> - `ensure-branch.sh` 负责切过去，**在采集/生成之前调用**（`daily.sh` 第一步；周报提示词的步骤 0）。
>   为什么必须这么早：`collect.py` 读写 `day-info/state.json`（45 天 URL 去重台账），
>   在落后的分支上跑会读到过期台账、去重静默失效；而且等到发布时台账已「两分支不同 + 有本地改动」，
>   git 会拒绝切换、任务直接失败。工作区干净的时候切，才是稳的。
> - 原分支记在 `.git/dayinfo-orig-ref`，由流程最后一步 `publish.sh` 切回（分支或游离 HEAD 都能还原）。
> - 本地没有 `day-info` 就从 `origin/day-info` 创建。
>
> 三种情况不硬来，原地罢工返回 1：
> ① 有进行中的 git 操作（merge / rebase / cherry-pick / bisect）——切分支会毁现场；
> ② `git checkout` 拒绝切换（工作区有会被覆盖的本地改动）——**绝不用 `-f` 强切**；
> ③ 本地与 `origin` 都没有 `day-info` 分支。
>
> 退回「不在目标分支就拒绝」的老行为：`DAYINFO_AUTO_SWITCH=0`。
> `publish.sh` 只提交 `day-info/` 路径，暂存区里的其它文件不会被卷进这次提交。
>
> ⚠️ **脚本必须在 `day-info` 分支上也是最新的**：切过去之后，后续步骤跑的是 `day-info` 上那份脚本。
> 改完脚本只提交到 `main` 而不同步到 `day-info`，实际执行的仍是旧版本。

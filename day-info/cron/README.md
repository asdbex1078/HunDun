...
**提示词本身不依赖任何平台，而且 2026-09-22 起连路径都不用替换了**——
`prompt-*.md` 里全是仓库内相对路径，只要把任务的**工作目录设成仓库根目录**就能原样跑。
（`agent-cron-jobs.baseline.json` 是给别的项目照抄的模板，那里仍保留 `<REPO_ROOT>` 占位符。）

- **在 WorkBuddy 里**：新建定时任务，把工作目录设为仓库根目录，提示词复制 `prompt-*.md` 中代码块的内容，原样粘贴即可。
